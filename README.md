...
flowchart TD
    ...
    H --> I["worker_node"]
    I --> K["critic_node"]
    K -->|"не approved и retry_count < MAX_CRITIC_RETRIES (1)"| I
    K --> J["validator_node"]
...
Цикл **critic ↔ worker**: в `critic_node` при отклонении результатов worker повторно ставится `Send(worker, …)` пока `task.retry_count < MAX_CRITIC_RETRIES` (в коде `MAX_CRITIC_RETRIES = 1`) и есть инструкции для доработки — не более **одного** дополнительного запуска worker на ту же задачу; затем поток идёт в **validator**.

Основной цикл исполнения:

```text
scheduler -> worker -> critic <-> worker (до 1 ретрая critic) -> validator -> replanner
...
- **`allowed_libraries`**: либо множество имён модулей, разрешённых к импорту, либо **словарь** `{имя_в_globals: объект}` — в этом случае объекты попадают в `globals` песочницы (типичный пример: `{"pd": pd, "px": px}`), а множество разрешённых имён строится по **ключам** словаря.
- **`python_analysis`**: нативный LangChain tool, который фабрика добавляет worker-у автоматически. Он компилирует и выполняет Python-код в sandbox, требует создать `target_variable`, возвращает JSON с `generated_code`, preview результата, stdout/stderr, traceback, `possible_causes`, `solution_options` и `retry_guidance`.
- **`get_installed_packages()`**: статический метод, возвращает `{имя_дистрибутива: версия}` через `importlib.metadata.distributions` (аналог `pip freeze`, без subprocess). **Worker** перед каждым ReAct-запуском вызывает `sandbox.get_installed_packages()` и вставляет результат в системный промпт блоком `<available_python_packages>` (см. `_format_installed_packages` в `planner_agent/agent_nodes/worker_node.py`), чтобы модель знала, какие пакеты реально можно импортировать при вызове `python_analysis`.
...
```json
{
  ...
  "rationale": "Skill помогает выбрать источники, порядок анализа, проверки результата и suggested_skills."
}
```
...
`<planner_loaded_skills>` используется только как методика планирования: он помогает выбрать источники, зависимости, recovery-стратегии, ожидаемые результаты и `suggested_skills`. Skill не заменяет фактические данные.
...
Если `needs_revision=true`, reviewer feedback фиксируется для диагностики. В текущей конфигурации `PLAN_REVIEW_REVISION_ATTEMPTS = 0`, поэтому candidate plan используется без автоматической revision-попытки.
...
1. `<task_contract>` — `expected_output`, `required_artifacts`, `suggested_tools`, `suggested_skills`.
...
- использовать `python_analysis` для расчетов по уже доступным данным;
...
- Если `approved=false` и выполняются условия ретрая (`task.retry_count < MAX_CRITIC_RETRIES`, в коде лимит **1**, и есть текст доработки), граф снова отправляет ту же задачу в **worker** (цикл «диалог» critic ↔ worker).
...
**Цикл critic ↔ worker:** не более `MAX_CRITIC_RETRIES` повторных отправок в worker (в коде лимит **1**), при наличии непустых `improvement_instructions`.
...
4. **Plan review** (`PlanReview`): reviewer проверяет candidate plan; автоматическая пересборка сейчас отключена (`PLAN_REVIEW_REVISION_ATTEMPTS = 0`).
...
- Critic **после каждого** worker-шага: приёмка результата до validator; при отклонении — до одного ретрая worker с инструкциями в `task.config`.
...
| `python main_sandbox_code_example.py` | Демо `ClientPythonSandbox` и выполнения Python-кода по данным из CSV. |
...
| `python main_ui_agent_server.py` | Поднимает FastAPI с реальным `ResearchAgent` (fake Spark + нативный `python_analysis`). |
| `python examples/branch_from_graph_step_demo.py` | Проверка `LineageService.branch_from` для произвольного шага графа (без UI). |

Для сценариев с реальной моделью нужны ваши ключи и рабочий `model.py`. Для нового режима `main_ui_agent_server.py` отдельный MCP-сервер генерации кода не нужен: расчеты выполняет `python_analysis` в локальном sandbox.
...
| `tools` | Список внешних LangChain tools. Нативный `python_analysis` добавляется фабрикой отдельно; legacy-имена из `code_generator_tool_names` скрываются из списка worker tools. |
| `prompts` | Экземпляр `AnalysisAgentPrompts`; по умолчанию встроенные шаблоны из `prompts.py`. |
| `code_generator_tool_names` | Legacy-множество имён внешних генераторов кода. По умолчанию `{"generate_python_code"}`; если такое имя попало в `enabled_tool_names`, оно нормализуется в `python_analysis`. Для нового кода обычно передают `set()` или оставляют значение по умолчанию без добавления внешнего generator tool. |
...
| `stream_console` | Включает диагностический stream-режим graph. Сейчас служебные stream-updates почти не печатаются, основные следы сохраняются в lineage/artifacts. |
...
Поддерживаются четыре основных способа сборки SDK-агента:

1. **`ResearchAgent` из `BaseChatModel`, sandbox и tools** — основной путь для приложений и сервисов.
2. **`ResearchAgent` с готовым `graph`** — путь для тестов, MVP и кастомной сборки LangGraph без модели и sandbox в конструкторе facade.
3. **Прямая фабрика `planner_agent(...)`** — когда нужен только скомпилированный LangGraph workflow без удобного read API facade.
4. **FastAPI `create_app(...)`** — HTTP-слой поверх уже собранного `ResearchAgent` и тех же сервисов lineage/artifacts/skills.

### Минимальная инициализация через ResearchAgent
...
Важно: агент не обязан получать DataFrame в sandbox при старте. Базовый путь - загрузка данных через source/export tools. Sandbox нужен для исполнения Python-кода через `python_analysis` и хранения переменных, созданных в ходе исследования.

`code_generator_tool_names=set()` явно показывает актуальный режим: внешнего генератора кода нет, а нативный `python_analysis` будет добавлен фабрикой автоматически. Если параметр не передавать, поведение также остается совместимым, но legacy-имя `generate_python_code` будет считаться скрываемым именем старого генератора.

### Инициализация модели без хранения ключей в коде

`ResearchAgent` принимает любой `BaseChatModel`. Ключи лучше брать из переменных окружения, а не хранить в `model.py` или README.

OpenRouter/OpenAI-compatible пример:
...
## Инициализация Python-анализа

Новый runtime не требует отдельный `generate_python_code` tool. Фабрика `planner_agent(...)` всегда добавляет worker-у нативный инструмент `python_analysis`, который принимает готовый Python-код и выполняет его в `ClientPythonSandbox`.
...
Правило использования: source/export tools получают данные из баз, Spark или файлов с фильтрами на стороне источника. `python_analysis` нужен для вычислений, join, агрегатов, статистики и подготовки визуализаций по уже полученным данным, artifacts или sandbox-переменным.

Если в старой интеграции еще есть tool `generate_python_code`, не добавляйте его в `tools` для нового режима. Оставьте вычисления на `python_analysis`, а `code_generator_tool_names` используйте только как совместимость whitelist-имен.

### Инициализация с готовым graph

Этот режим удобен для тестов и кастомных сценариев, где graph уже собран отдельно. В конструкторе `ResearchAgent` тогда не нужны `model`, `sandbox` и `tools`, но read API остается доступным через переданные сервисы.
...
lineage_service = LineageService("examples/runs")
artifact_service = ArtifactService("examples/runs")
...
### Прямая сборка LangGraph workflow

Если facade не нужен, можно собрать только workflow:
...
В этом режиме read API (`get_run_result`, `get_run_graph`, `list_artifacts`) не добавляется автоматически; его предоставляет facade `ResearchAgent`.
...
AnaliticAgenticPlatform/
  planner_agent/          # ядро: граф, узлы, сервисы, промпты
    ...
    tools/                # python_analysis, artifact read tools, wrappers, skill_list/skill_view
    ...
    prompts.py
  sandbox/                # ClientPythonSandbox и sandbox runtime для Python-анализа
...
- добавляет нативный `python_analysis` и скрывает legacy code-generator tools;
...
- получает нативный `python_analysis` для расчетов в sandbox;
- формирует prompt;
...
- python_analysis: выполнить расчеты по уже доступным данным
</available_tools>
...
WorkerPayload(
    task=Task(
        ...
        dependencies=["2"],
        suggested_tools=["python_analysis"],
...
- `python_analysis` используется для расчетов по уже доступным данным;
