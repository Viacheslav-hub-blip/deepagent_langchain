...
значения) НЕ делегируй повторный `load_data`. Обрабатывай данные через `execute_python_code`.
...
Каждый `load_data` — тяжёлая операция выгрузки из Spark. Middleware сохраняет
**полный** табличный результат каждого успешного `load_data` в pickle (путь в tool output
...
- **НЕ** вызывай повторный `task(data-retrieval-agent)` / `load_data` с более узкими
...
Новый `load_data` допустим только когда нужны **другие** данные: другая таблица, другой
...
результат `load_data` в `.pkl`), читай и обрабатывай его через `execute_python_code`
(`describe_pickle_file`, `read_pickle_file`, `rows_to_dataframe`). Для урезанной выборки
из уже загруженного набора — только код по pickle, без повторного `load_data`
