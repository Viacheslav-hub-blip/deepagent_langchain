...
_TRANSPARENCY_NOTE = (
    ...
    "файле (читать/фильтровать через execute_python_code, не новым read_table). "
    "Обычный select возвращает строки как есть, БЕЗ устранения дублей: "
    "уникальные значения выводи сам по полному набору. group_by + aggregations — "
    "необязательный способ сразу получить группы со счётчиками."
)
